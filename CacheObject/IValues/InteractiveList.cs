...
namespace HalfbyteMedia.OnARoll.MakioClient.CacheObject.IValues
{
    public class InteractiveList : InteractiveValue, ICellPoolDataSource<CacheListEntryCell>, ICacheObjectController
    {
        ...
        private void CheckGenericIList(object value)
        {
            try
            {
                ...
                if (IsWritableGenericIList)
                {
                    ...
                    {
                        MakioClient.LogWarning($"Failed to find indexer property for IList<T> type '{type.FullName}'!");
                        IsWritableGenericIList = false;
                    }
                }
            }
            ...
            {
                MakioClient.LogWarning($"Exception processing IEnumerable for IList<T> check: {ex.ReflectionExToString()}");
                IsWritableGenericIList = false;
            }
        }
        ...
        public void TrySetValueToIndex(object value, int index)
        {
            ...
            {
                MakioClient.LogWarning($"Exception setting IList value: {ex}");
            }
        }
